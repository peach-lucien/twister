...
SEED=42 # random seed
batch_size = 10 # batch size for learning
...
train_data = dataset(xtrain, ytrain, tfms=1, dtype=torch.float)
test_data = dataset(xtest, ytest, tfms=0, dtype=torch.float)
...
criterion = torch.nn.BCELoss() # torch.nn.BCEWithLogitsLoss() # torch.nn.BCELoss() 
